...
async function getBrowser(): Promise<Browser> {
  ...
  if (isProduction && browserlessApiKey) {
    // Production - connect to Browserless.io with AU residential proxy
    const browserlessUrl =
      `wss://production-sfo.browserless.io?token=${encodeURIComponent(browserlessApiKey)}` +
      `&proxy=residential&proxyCountry=au&proxySticky=true`;

    browserInstance = await puppeteer.connect({
      browserWSEndpoint: browserlessUrl,
    });

    console.log('[Crawler] Connected to Browserless.io with AU residential proxy');
...
export async function closeBrowser(): Promise<void> {
  ...
  try {
    // For Browserless connections, use disconnect (non-async)
    const browserlessApiKey = process.env.BROWSERLESS_API_KEY;
    const isProduction = process.env.NODE_ENV === 'production' || !!process.env.VERCEL;

    if (isProduction && browserlessApiKey) {
      browserInstance.disconnect();
      console.log('[Crawler] Disconnected from Browserless.io (AU proxy)');
    } else {
  ...
  try {
    ...
    const extracted = await page.evaluate((base) => {
      ...
      for (const link of allLinks) {
        ...
        const teamEls = link.querySelectorAll(
          '[class*="team"], [class*="participant"], [class*="competitor"], [class*="name"]'
        );
        if (teamEls.length >= 2) {
        ...
        if (homeTeam && awayTeam && homeTeam !== awayTeam) {
          ...
          if (homeTeam.length > 2 && awayTeam.length > 2 && homeTeam.length < 50 && awayTeam.length < 50) {
            results.push({
              homeTeam,
              awayTeam,
              eventUrl: href,
...
export async function crawlBookmakerSport(bookmakerName: string, sport: string): Promise<CrawlResult> {
  ...
  try {
    ...
    // Set Australian headers
    await page.setExtraHTTPHeaders({ 'Accept-Language': 'en-AU,en;q=0.9' });

    // Spoof Australian geolocation (Sydney)
    try {
      const context = browser.defaultBrowserContext();
      await context.overridePermissions(bookmaker.baseUrl, ['geolocation']);
      await page.setGeolocation({ latitude: -33.8688, longitude: 151.2093, accuracy: 100 });
      console.log('[Crawler] Set geolocation to Sydney, Australia');
    } catch {
      console.log('[Crawler] Could not set geolocation, continuing anyway');
    }
    ...
    // Check for geo-block - fail loudly instead of silently returning 0 events
    const finalUrl = page.url();
    const title = await page.title().catch(() => '');

    if (finalUrl.includes('location-error') || finalUrl.includes('geo-block') || finalUrl.includes('restricted')) {
      throw new Error(
        `[GeoBlock] Landed on ${finalUrl} (title="${title}"). ` +
          `Proxy likely not AU or residential proxy not enabled on your Browserless plan.`
      );
    }

    console.log('[Crawler] Successfully loaded:', finalUrl);
...
export async function crawlBookmaker(bookmakerName: string): Promise<CrawlResult[]> {
  ...
  for (const sport of Object.keys(bookmaker.sports)) {
    const r = await crawlBookmakerSport(bookmakerName, sport);
    results.push(r);
...
export async function crawlSportAcrossBookmakers(sport: string): Promise<CrawlResult[]> {
  ...
  for (const bookmaker of BOOKMAKERS) {
    if (bookmaker.sports[sport]) {
      const r = await crawlBookmakerSport(bookmaker.name, sport);
      results.push(r);
      await new Promise((resolve) => setTimeout(resolve, 2000));
    }
  }
...
export async function lookupEventUrl(
  ...
  await dbConnect();
  const r = await EventUrl.findEventUrl(bookmaker, homeTeam, awayTeam, sport);
  return r?.eventUrl || null;
}
